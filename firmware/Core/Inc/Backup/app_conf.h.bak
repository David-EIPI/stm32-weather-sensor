...
typedef enum
{
  ...
  CFG_TIM_ZIGBEE_APP_BME280_REPEAT,
  CFG_TIM_ZIGBEE_STOP_FASTPOLL_ONESHOT,
...
/* 1 - use AHT20 to read humidity, 0 - use bme280 */
#define USE_AHT20 1
...
#define MODEL_NAME "\x08" "Weather1"
#define LOCATION "\x0f" "Weather station"
 ...
 * Default report settings in ZHA are over conservative, and sometimes even fail to be
 ...
 * STMs implementation of Zigbee Sleepy End Device (SED) is fairly stringent.
 * It transitions to low power Stop2 mode as soon as association happens and
 * limits data exchanges to intervals defined by ZED_SLEEP_TIME_30S.
 * ZHA expects information about endpoints, clusters and attributes immediately after
 * association. With default SED wake-up periods ZHA times out and does not setup
 * the device correctly.
 * Setting this macro adds extra fast polling period immediately after joining the network.
 * If not using ZHA, this macro can be disabled to save power.
 * Fast polling is not used after start from persistence. Supposedly, ZHA should have the device
 * already configured at that time.
 *
